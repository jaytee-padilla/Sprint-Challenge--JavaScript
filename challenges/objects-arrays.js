...
const universities = graduates.map(graduateObj => graduateObj.university);
...
const contactInfo = graduates.map(graduateObj => `${graduateObj.first_name} ${graduateObj.email}`);
console.log(contactInfo);


/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called uni that contains them all. Log the result. */
/*** Ugly Code - Higher Order Function Chain***/
// const uni = graduates.filter((graduateObj) => {
//   return graduateObj.university.includes("Uni")
// }).map((graduateObj) => {
//   return graduateObj.university
// });

/*** Cleaner Code - Higher Order Function Chain ***/
const includesUni = (graduateObj) => {
  return graduateObj.university.includes("Uni");
}
const getUniversityName = (graduateObj) => {
  return graduateObj.university;
}

const uni = graduates
.filter(includesUni)
.map(getUniversityName);
console.log(uni);
...
*/
const animalNames = [];
zooAnimals.forEach(function(animalObj) {
  animalNames.push(`Name: ${animalObj.animal_name}, Scientific: ${animalObj.scientific_name}`);
});
...
const lowerCase = zooAnimals.map(animalObj => `${animalObj.animal_name.toLowerCase()}`);
...
*/
const largerPopulation = zooAnimals.filter(animalObj => animalObj.population < 5);
...
*/
const populationTotal = zooAnimals.reduce((accumulator, animalObj) => accumulator + animalObj.population, 0);
